...
signed short main(void)
{
   ...
   ssReadFile(&stConfigurationFile, &stConfiguration, sizeof(ST_CONFIG), 1);
   ...
   {
     ssRet = ssReadFile(&stConfiguration.stObjectFile, &stObjectArray[ssCount], sizeof(ST_OBJECT), ssCount);
   ...
   {
     ssRet = ssReadFile(&stConfiguration.stConditionFile, &stConditionArray[ssCount], sizeof(ST_COND), ssCount);
   ...
   ssRet = ssReadFile(&stConfiguration.stRoomFile, &stRoomRecord, sizeof(ST_ROOM), stPlayer.ssCurrentRoom );
